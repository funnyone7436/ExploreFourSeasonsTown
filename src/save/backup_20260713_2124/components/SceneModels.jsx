...
export default function SceneModels({
  ...
  const airBalloon = useGLTF('/glb/airBalloon.glb') // 1. Load the Air Balloon
  ...
  // 6. Air Balloon Math (Positioned exactly between FallHouse and BigTree)
  const airBalloonAngle = offsetAngle + (Math.PI) - 1.6
  const airBalloonPos = [getX(airBalloonAngle)-40, height , getZ(airBalloonAngle)-40] // Floating in the air
  const airBalloonRot = [0, airBalloonAngle + Math.PI, 0]
  const airBalloonScale = a // Adjust this scale up or down as needed
...
useGLTF.preload('/glb/airBalloon.glb') // 2. Preload the new model