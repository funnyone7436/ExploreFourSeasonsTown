...
export default function SceneModels({
  ...
  const squirrel = useGLTF('/glb/Squirrel.glb') // 1. Load the Squirrel

  const bikeRef = useRef()
  const catRef = useRef()
  const girlRef = useRef() 
  const squirrelRef = useRef() // 2. Reference for the Squirrel
  ...
  const { actions: squirrelActions } = useAnimations(squirrel.animations, squirrelRef) // 3. Squirrel animations
  ...
  // Adjust the "+ 5" up or down to dial in the height!
...
useGLTF.preload('/glb/Squirrel.glb') // 4. Preload Squirrel