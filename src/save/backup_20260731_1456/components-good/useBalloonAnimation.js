        ...
        if (child.userData.isFlying) {
          ...
          for (let j = 0; j < flyaways.length; j++) {
            if (i !== j && flyaways[j].userData.isFlying) {
              ...
              // Calculate real distance between this balloon and the other balloon
              const distance = Math.sqrt(dx * dx + dy * dy + dz * dz);
              
              // If they are closer than 1.5 units, apply a repelling force
              if (distance < 1.5 && distance > 0.01) {
                const repelStrength = 0.03; // Adjust this to make them push away harder or softer
                child.position.x += (dx / distance) * repelStrength;
                child.position.z += (dz / distance) * repelStrength;
              }
            }
          }
          // --- END BOID SEPARATION LOGIC ---
