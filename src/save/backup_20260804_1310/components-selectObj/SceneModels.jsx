...
export default function SceneModels({
  ...
  useEffect(() => {
    ...
    // Send names to the UI
    if (setAllMeshes) setAllMeshes(extractedNames)
  }, [fallHouse, bookStore, bigTree, fallHouseTrees, setAllMeshes])

  // 3. NEW EFFECT: Highlight the active mesh when selected from UI or Canvas
  useEffect(() => {
  ...
  const handlePointerDown = (e) => {
    e.stopPropagation() 
    if (e.object.isMesh && setSelectedMesh) {
  ...
  return (
    <group>
      {/* 4. WRAP StaticScenery to capture pointer events for the 4 models inside it */}
      <group 
