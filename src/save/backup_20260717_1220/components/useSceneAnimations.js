...
export default function useSceneAnimations(
  ...
  clipEndAngle,
  bookStoreRef,
  bigTreeRef // 1. ADDED REFERENCE HERE
) {
  ...
  useFrame((state) => {
    ...
    if (bigTreeRef && bigTreeRef.current) {
      ...
      const bigTreeConfig = {
        // Changed to 'flower' type and added arcHeight/swing. Speed lowered to 1.5 for a smooth arch motion.
        'Leaf_DarkGreen_Bent':       { limit: defaultLimit, speed: 0.5, arcHeight: 0.2, swing: 0.2, type: 'flower', key: 'DGB' },
        'Leaf_DarkGreen_Long':       { limit: higherLimit,  speed: 0.7, amplitude: 0.02, type: 'petal', key: 'DGL' },
        'Leaf_DarkGreen_Standard':   { limit: defaultLimit, speed: 0.1, amplitude: 0.38, type: 'petal', key: 'DGS' },
        'Leaf_DarkGreen_Wide':       { limit: defaultLimit, speed: 0.4, amplitude: 0.61, type: 'petal', key: 'DGW' },
        'Leaf_LightGreen_Bent':      { limit: defaultLimit, speed: 1.8, amplitude: 1.75, type: 'petal', key: 'LGB' },
        'Leaf_LightGreen_Long':      { limit: higherLimit,  speed: 0.5, amplitude: 0.29, type: 'petal', key: 'LGL' },
        'Leaf_LightGreen_Standard':  { limit: defaultLimit, speed: 0.9, amplitude: 0.02, type: 'petal', key: 'LGS' },
        'Leaf_LightGreen_Wide':      { limit: defaultLimit, speed: 1.1, amplitude: 1.88, type: 'petal', key: 'LGW' },
        'Leaf_MidGreen_Bent':        { limit: defaultLimit, speed: 0.6, amplitude: 0.06, type: 'petal', key: 'MGB' },
        'Leaf_MidGreen_Long':        { limit: higherLimit,  speed: 0.9, amplitude: 0.31, type: 'petal', key: 'MGL' },
        'Leaf_MidGreen_Standard':    { limit: defaultLimit, speed: 0.3, amplitude: 0.09, type: 'petal', key: 'MGS' },
        'Leaf_MidGreen_Wide':        { limit: defaultLimit, speed: 0.9, amplitude: 0.24, type: 'petal', key: 'MGW' },
        'Leaf_YellowGreen_Bent':     { limit: higherLimit,  speed: 1.8, amplitude: 1.86, type: 'petal', key: 'YGB' },
        'Leaf_YellowGreen_Long':     { limit: defaultLimit, speed: 0.5, amplitude: 0.48, type: 'petal', key: 'YGL' },
        'Leaf_YellowGreen_Standard': { limit: defaultLimit, speed: 0.9, amplitude: 0.35, type: 'petal', key: 'YGS' },
        'Leaf_YellowGreen_Wide':     { limit: defaultLimit, speed: 0.1, amplitude: 0.69, type: 'petal', key: 'YGW' }
      };
      ...
      if (bigTreeRef.current.userData.isSetup === undefined) {
        ...
        Object.keys(bigTreeConfig).forEach(prefix => {
          ...
          selectedItems.forEach(child => {
            ...
            // Set up based on type (just like bookstore flowers)
            if (config.type === 'petal') {
              child.geometry = child.geometry.clone();
              child.geometry.center();
              const seed = child.position.x + child.position.y + child.position.z;
      ...
      for (let i = 0; i < activeItems.length; i++) {
        ...
        if (child.userData.type === 'petal') {
          // Rotation logic for standard leaves
          const oscillation = Math.sin(time * child.userData.speed + child.userData.phaseOffset) * child.userData.amplitude;
          child.rotation.y = child.userData.baseRot.y + oscillation;
          child.rotation.x = child.userData.baseRot.x + (oscillation * 0.2);
        } else {
          // Arch logic for 'flower' type leaves
          const t = time * child.userData.speed;
          child.position.x = child.userData.basePos.x + Math.sin(t) * child.userData.swing;
          child.position.y = child.userData.basePos.y + Math.abs(Math.sin(t)) * child.userData.arcHeight;
        }
