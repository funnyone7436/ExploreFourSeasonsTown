...
export default function OceanAudio() {
  ...
  useFrame(({ camera }, delta) => {
    ...
    // Define our distinct viewing zones
    // CHANGED: Added the 1.9 PI to 2.0 PI range for the ocean
    const oceanInView = (angle >= 0 && angle <= 0.4 * Math.PI) || (angle >= 1.9 * Math.PI && angle <= 2.0 * Math.PI);
    const birdInView = angle > 0.4 * Math.PI && angle <= 0.8 * Math.PI; 
    const cicadaInView = angle >= 1.0 * Math.PI && angle <= 1.4 * Math.PI; 
    const fallInView = angle >= 1.5 * Math.PI && angle < 1.9 * Math.PI; 
    ...
    const newOceanVol = THREE.MathUtils.lerp(oceanAudioRef.current.volume, targetOceanVolume.current, delta * 2.0);
    const newBirdVol = THREE.MathUtils.lerp(birdAudioRef.current.volume, targetBirdVolume.current, delta * 2.0);
    const newCicadaVol = THREE.MathUtils.lerp(cicadaAudioRef.current.volume, targetCicadaVolume.current, delta * 2.0);
    const newFallVol = THREE.MathUtils.lerp(fallAudioRef.current.volume, targetFallVolume.current, delta * 2.0); 
	const newSpringVol = THREE.MathUtils.lerp(springAudioRef.current.volume, targetSpringVolume.current, delta * 1.0); // ADDED: Slower fade for background music
	...
	springAudioRef.current.volume = THREE.MathUtils.clamp(newSpringVol, 0, 1); // ADDED
  });
