...
import useCelestialAnimation from './useCelestialAnimation' // <-- NEW HOOK
...
// --- CELESTIAL ANIMATIONS ---
  ...
  // Define exact keyframes: [ Angle, Height, Scale ]
  // Make sure the array starts at 0 and ends at Math.PI * 2.0
  const sunPath = [
    [Math.PI * 0, -20, a],                   // 0 PI: Hidden underground
    [Math.PI * 0.15, -20, a],      // 0.15 PI: Still hidden
    [Math.PI * 0.15, 30, a],      // 0.15 PI: Still hidden
    [Math.PI * 0.2, 30, a * 1.6],    // 0.2 PI: Rises up to 30 height, full scale
    [Math.PI * 0.4, 60, a * 2],    // 0.4 PI: Stays at exactly 30 height
    [Math.PI * 0.6, 65, a * 2],  // 0.6 PI: Climbs higher to 45, scales up
    [Math.PI * 1.0, 60, a * 2],    // 1.0 PI: Drops back down to 30
    [Math.PI * 1.6, 60, a * 1.6],      // 1.05 PI: Sets quickly back underground
    [Math.PI * 1.8, 60, a],      // 1.05 PI: Sets quickly back underground
    [Math.PI * 1.9, 30, a],      // 1.05 PI: Sets quickly back underground
    [Math.PI * 1.95, 10, a],      // 1.05 PI: Sets quickly back underground
    [Math.PI * 2.0, -20, a]
  ]

  const moonPath = [
    [Math.PI *0, -20, a],                   
    [Math.PI * 1.15, -20, a],      // Hides while sun is out
    [Math.PI * 1.2, 60, a * 5],  // 1.2 PI: Moon rises to 30 height
    [Math.PI * 1.4, 60, a * 1.5],  // 1.4 PI: Stays at exactly 30
    [Math.PI * 1.8, 60, a * 1.5],  // 1.8 PI: Still at 30
    [Math.PI * 1.85, 60, a],      // 1.85 PI: Moon sets
    [Math.PI * 1.9, 30, a],      // 1.85 PI: Moon sets
    [Math.PI * 2.0, -20, a]        
  ]
  ...
  useCelestialAnimation(sunRef, {
    ...
    center: [0, 0, 0], 
    pathPoints: sunPath,
    initialProgressAngle: Math.PI * 0.2 // Load the game exactly at Sun's 0.2 PI
  })
  ...
  useCelestialAnimation(moonRef, {
    ...
    center: trainCenter, 
    pathPoints: moonPath,
    initialProgressAngle: Math.PI * 0.2 // Keep time synced with the sun!
  })
  // ------------------------------------------
  ...
  return (
    <group>
      ...
      {/* --- MOUNT CELESTIAL OBJECTS TO THEIR REFS --- */}
      <group ref={sunRef}>
