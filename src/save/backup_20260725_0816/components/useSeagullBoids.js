...
export default function useSeagullBoids(refs) {
  ...
  useFrame((state) => {
    ...
    refs.current.forEach((ref, i) => {
      ...
      // Apply forces
// Apply forces
		bird.velocity.add(alignment.multiplyScalar(0.02));   // Lowered from 0.04
		bird.velocity.add(cohesion.multiplyScalar(0.005));   // Lowered from 0.01
		bird.velocity.add(separation.multiplyScalar(0.05));  // Lowered from 0.09
