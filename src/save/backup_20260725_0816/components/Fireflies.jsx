...
export default function Fireflies({ count = 30 }) {
  ...
  useFrame((state) => {
    ...
    refs.current.forEach((ref, i) => {
      ...
      // FLOCK PULL: Split the axes. Strong vertical pull, weak horizontal pull.
      const toTarget = targetPos.clone().sub(pos).normalize();
      toTarget.x *= 0.002; // Very weak horizontal pull allows them to wander left/right
      toTarget.y *= 0.03;  // Strong vertical pull keeps them hovering near the ground
      toTarget.z *= 0.002; // Very weak horizontal pull allows them to wander forward/back
      // Apply forces
