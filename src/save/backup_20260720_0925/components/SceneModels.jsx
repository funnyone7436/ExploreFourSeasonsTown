...
// --- Import the new separated hooks and configs ---
...
// SUB-COMPONENT: Handles cloning and animating a single bird
function AnimatedSeagull({ seagullGltf, birdRef, scale }) {
  // 1. Deep clone the scene AND its skeletal bones so animations work
  const clonedScene = useMemo(() => SkeletonUtils.clone(seagullGltf.scene), [seagullGltf.scene]);
  
  // 2. Bind the original animations to this specific bird's cloned scene
  const { actions } = useAnimations(seagullGltf.animations, clonedScene);
  
  // 3. Auto-play ALL animations when the bird loads
  useEffect(() => {
    // Grab all available animation tracks in the GLTF
    const actionNames = Object.keys(actions);
    
    if (actionNames.length > 0) {
      // Add a slight random delay (0 to 1 second) so the flock doesn't flap in perfect robotic sync
      setTimeout(() => {
        // PLAY BOTH SIDES: Loop through every track (e.g., Track 1 and Track 2) and play them
...
useMemo(() => {
    // 1. Sun Material (Procedural Plasma)
...
// 2. Moon Material (Natural crater look, White-Yellow color)
    const moonMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          // Use noise to create subtle crater-like texturing
          float n = noise(vPosition * 4.0);
          
          // White-Yellow color palette
          vec3 brightWhiteYellow = vec3(1.0, 0.98, 0.85); // Bright pale yellow/white
          vec3 warmCraterShadow = vec3(0.85, 0.80, 0.60); // Slightly darker, warm golden shadow
          
          // Mix with a very subtle variance
  ...
  // --- NEW TRAIN MATERIAL OVERRIDE ---
  useMemo(() => {
    train.scene.traverse((child) => {
      if (child.isMesh && child.name.startsWith('Body_Cream_')) {
        // Clone the material so it doesn't accidentally affect other objects sharing it
        child.material = child.material.clone()
        
        // Apply white color and white emission
        child.material.color = new THREE.Color(0xffffff)
        child.material.emissive = new THREE.Color(0xffffff)
        
        // Adjust intensity as needed (values > 1 work well if you have post-processing bloom)
  ...
  // --- TRIGGER THE MODULAR ANIMATION HOOKS ---
  ...
  // Position it slightly offset from the sun
  const moonPos = [sunPos[0] + 5, sunPos[1] + 5, sunPos[2]]; 
  const moonRot = [ Math.PI, sunAngle-Math.PI/2, 0];
  const moonScale = a * 1.5; // Adjust size as preferred
  ...
  return (
    <group>
	  ...
	  {/* --- ADDED MOON HERE --- */}
      <group position={moonPos} rotation={moonRot} scale={[moonScale, moonScale, moonScale]}>
...
{/* Replaces the old single seagull primitive and the <Clone /> map */}
