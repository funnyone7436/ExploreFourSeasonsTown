...
// --- CELESTIAL ANIMATIONS ---
  ...
  const sunPath = [
    ...
    [Math.PI * 0.19, -20, a * 5.6, 0.5, -1], // Starts hidden, waits for 1st Enter to rise
...
// SUN
  useCelestialAnimation(sunRef, {
    ...
    // ADD THIS BLOCK
	onUpdate: (angle) => {
      // Start growing as soon as it rises (0.0 PI) and finish growing by 0.2 PI
      const start = Math.PI * 0.0; 
      const end = Math.PI * 0.2;
      let beamScale = 0.01;
      
      if (angle >= start && angle <= end) {
        const progress = (angle - start) / (end - start);
        beamScale = 0.01 + (1.0 - 0.01) * progress;
      } else if (angle > end) {
        beamScale = 1.0; 
      }
  ...
  const girlPos = [getX(girlAngle)+10, height + 7., getZ(girlAngle)-10] 
  const girlRot = [0, girlAngle + Math.PI+0.5, 0]
  const girlScale = a*0.46 

  const fenceAngle = girlAngle - .200  
  const fencePos = [getX(fenceAngle)-8, height+7.4, getZ(fenceAngle)+8] 
