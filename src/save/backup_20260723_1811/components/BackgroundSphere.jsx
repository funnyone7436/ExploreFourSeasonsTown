...
export default function BackgroundSphere({ 
  ...
  // Tweak these values to perfectly align the 3D model with the painted texture
  const sea0OffsetPos = [0, -0.3, 0]; // Change the middle value (e.g., 0.5 or -0.5) to move it up/down
  const sea0OffsetRot = [0, Math.PI+0.215, 0]; // Change the middle value (e.g., Math.PI / 4 or 0.1) to spin it
  ...
  // Load all 3D wave objects, but we are focusing on sea0 matching Sea 4!
  const sea0Gltf = useGLTF('/glb/sea0.001.glb')
  const sea1Gltf = useGLTF('/glb/sea1.001.glb')
  const sea2Gltf = useGLTF('/glb/sea2.001.glb')
  const sea3Gltf = useGLTF('/glb/sea3.001.glb')
  
  useMemo(() => {
    sea0Gltf.scene.traverse((child) => {
      if (child.isMesh) {
        // Clone the material first so it doesn't accidentally affect other objects
        child.material = child.material.clone();
        
        // Put whatever hex color you want here!
        child.material.color = new THREE.Color('#12b5de'); 
        
        // Optional: If you want to make it slightly transparent to blend with the image
  ...
  // 4. Rotate AND bob each sea layer group (Both 2D and 3D sync together)
  useFrame((state) => {
  ...
  return (
    <group>
      ...
      {/* SEA 4 + sea0.001.glb (MOVED HERE to share speed/direction) */}
      <group ref={sea4Ref}>
        <mesh 
          scale={[-1, 1, 1]} 
          rotation={[0, Math.PI / 2, 0]} 
        ...
        {/* The 3D model with your adjustable offsets applied */}
...
useGLTF.preload('/glb/sea2.001.glb')
useGLTF.preload('/glb/sea3.001.glb')