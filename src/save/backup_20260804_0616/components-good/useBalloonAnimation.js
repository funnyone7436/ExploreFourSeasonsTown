...
// OPTIMIZATION: Define the static color array once outside the component 
// so it isn't recreated on every render or launch.
const BALLOON_COLORS = ['#ff4d4d', '#ffed4a', '#4add74', '#4a90e2', '#ff66c4']
  ...
  useFrame((state) => {
    ...
    if (bikeRef.current) {
        ...
        bikeRef.current.traverse((child) => {
          if (child.isMesh && (child.name.includes('Sphere') || child.name === 'Balloon')) {
            ...
            if (!isFlyawayMesh) {
              ...
              child.userData.basePosY = child.position.y
              // OPTIMIZATION: Calculate this offset exactly once during initialization instead of every frame
              child.userData.organicOffset = child.position.x * 5 + child.position.y * 5
      ...
      for (let i = 0; i < balloons.length; i++) {
        ...
        // Use the pre-calculated offset here
        const offset = child.userData.organicOffset;
        child.position.x = child.userData.basePosX + Math.sin(time * 2 + offset) * 0.15
        child.position.y = child.userData.basePosY + Math.cos(time * 1.5 + offset) * 0.15
      }
      ...
      if (time > bikeRef.current.userData.nextFlyTime) {
        ...
        for (let i = 0; i < flyaways.length && launched < countToFly; i++) {
          if (!flyaways[i].userData.isFlying) {
            ...
            // Pick a random color using the static global array
            const randomHex = BALLOON_COLORS[Math.floor(Math.random() * BALLOON_COLORS.length)];
      ...
      for (let i = 0; i < flyaways.length; i++) {
        ...
        if (child.userData.isFlying) {
          ...
          for (let j = 0; j < flyaways.length; j++) {
            if (i !== j && flyaways[j].userData.isFlying) {
              const other = flyaways[j];
              
              const dx = child.position.x - other.position.x;
              const dy = child.position.y - other.position.y;
              const dz = child.position.z - other.position.z;
              
              // OPTIMIZATION: Check the squared distance first (1.5 * 1.5 = 2.25)
              // This completely avoids running the heavy Math.sqrt() function for balloons that are far apart!
              const distSq = dx * dx + dy * dy + dz * dz;
              
              if (distSq < 2.25 && distSq > 0.0001) {
                // Only calculate the true distance if they are dangerously close
                const distance = Math.sqrt(distSq); 
                const repelStrength = 0.03; 
                child.position.x += (dx / distance) * repelStrength;
                child.position.z += (dz / distance) * repelStrength;
              }
            }
          }
