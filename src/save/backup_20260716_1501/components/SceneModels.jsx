...
export default function SceneModels({
  ...
  // 1. Load the new Bookstore front models
  ...
  // --- CLIPPING TUNNEL CONTROLS (in PI) ---
  ...
  // 16. TwoPersons Math (Placed in front of the BookStore)
  const twoPersonsAngle = offsetAngle + 0.90
  const twoPersonsPos = [getX(twoPersonsAngle) + 30, height + 5, getZ(twoPersonsAngle) - 30] 
  const twoPersonsRot = [0, twoPersonsAngle + Math.PI, 0]
  const twoPersonsScale = a * 0.5

  // 17. RedCar Math (Parked in front of the BookStore)
  const redCarAngle = offsetAngle - 0.1
  const redCarPos = [getX(redCarAngle) + 35, height + 4.6, getZ(redCarAngle) - 35] 
  const redCarRot = [0, redCarAngle + Math.PI + (Math.PI / 2), 0] // Rotated 90 degrees
  const redCarScale = a * 0.6
