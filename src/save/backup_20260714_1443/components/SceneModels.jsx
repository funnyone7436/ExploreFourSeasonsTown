...
export default function SceneModels({
  ...
  // 7. Cat Math (Now using its own independent angle)
  // Adjust -0.15 to slide it left or right along the circle relative to the bookstore (-0.12)
  const catAngle = offsetAngle + 0.68
  // Height is set to height+20 to match the bookstore's base height
  const catPos = [getX(catAngle)+28, height + 5, getZ(catAngle)-28] 
  // Add or subtract numbers from (catAngle + Math.PI) to rotate which way the cat faces
  const catRot = [0, catAngle + Math.PI, 0]
  const catScale = a * 0.7
