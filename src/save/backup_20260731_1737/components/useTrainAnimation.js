...
// OPTIMIZATION: Hoisted constant to avoid recalculating 60 times a second
const PI_2 = Math.PI * 2;

function getTrackHeight(targetAngle, points) {
  const normalizedAngle = ((targetAngle % PI_2) + PI_2) % PI_2
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (normalizedAngle >= angle1 && normalizedAngle <= angle2) {
      ...
      // OPTIMIZATION: Consolidated the smoothing math
      return height1 + (height2 - height1) * t * t * (3 - 2 * t)
    }
  }
  return 0
}
...
export default function useTrainAnimation(trainRef, trainScene, trainRadius, trainSpeed, trainCenter, gl, clipStartAngle, clipEndAngle) {
  ...
  // OPTIMIZATION: Cache values that don't need to be calculated every single frame
  ...
  useFrame((state) => {
    ...
    if (trainRef.current) {
      ...
      // OPTIMIZATION: Uses the cached forwardDistance variable here
