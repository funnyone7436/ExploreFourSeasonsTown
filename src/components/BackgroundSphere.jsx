...
// OPTIMIZATION: Pre-calculate static colors outside memory so they aren't recreated during traversal
const sea0Color = new THREE.Color('#12b5de');
const sea1Color = new THREE.Color('#74cee5');
...
export default function BackgroundSphere({ 
  ...
  // 1. The background texture loader using BASE_URL
  const [
    textureFront,
    textureSea1,
    textureSea2,
    ...
    `${import.meta.env.BASE_URL}frame_8k_season_front_d_1043.png`,
    `${import.meta.env.BASE_URL}frame_8k_season_sea_1_1043.png`,
    `${import.meta.env.BASE_URL}frame_8k_season_sea_2_1043.png`,
    `${import.meta.env.BASE_URL}frame_8k_season_sea_3_1043.png`,
    `${import.meta.env.BASE_URL}frame_8k_season_sea_4_1043.png`,
    `${import.meta.env.BASE_URL}frame_8k_season_back_d_1043.png`
  ])

  // Load remaining 3D wave objects using BASE_URL
  const sea0Gltf = useGLTF(`${import.meta.env.BASE_URL}glb/sea0.001.glb`)
  const sea1Gltf = useGLTF(`${import.meta.env.BASE_URL}glb/sea1.001.glb`)
  ...
  useMemo(() => {
    sea0Gltf.scene.traverse((child) => {
      if (child.isMesh) {
        ...
        child.material.color = sea0Color; 
        child.material.emissive = sea0Color; 
  ...
  useMemo(() => {
    sea1Gltf.scene.traverse((child) => {
      if (child.isMesh) {
        child.material = child.material.clone();
        child.material.color = sea1Color; 
        child.material.emissive = sea1Color; 
        child.material.emissiveIntensity = 0.5;
      }
    });
  }, [sea1Gltf.scene]);
  ...
  useFrame((state) => {
    // OPTIMIZATION: Read directly from state to avoid overhead
...
// Preload the GLTF objects using BASE_URL
useGLTF.preload(`${import.meta.env.BASE_URL}glb/sea0.001.glb`)
useGLTF.preload(`${import.meta.env.BASE_URL}glb/sea1.001.glb`)