...
// OPTIMIZATION: Pre-calculate constants outside the frame loop so the GPU isn't doing the same math 60x a second
const PI_1_8 = 1.8 * Math.PI;
const PI_0_1 = 0.1 * Math.PI;
...
export default function StaticScenery({
  ...
  const p = useMemo(() => {
    ...
    return {
      ...
      airBalloon: { 
        pos: [getX(offsetAngle + (Math.PI) - 1.6)-40, height-2, getZ(offsetAngle + (Math.PI) - 1.6)-40], 
  ...
  useFrame((state) => {
    ...
    // OPTIMIZATION: Using the pre-calculated constants
    const sweepAngle = PI_1_8 - Math.cos(t * 0.1) * PI_0_1;
